...
# Functions
# --------------------------------------------------------------------------- #
def beta_probe_post_detuned(omega, beta_s, beta_m, gamma, g=1.0):
    gamma = np.asarray(gamma, dtype=float)
    delta = gamma.sum() - omega
    eta = g**2 / (g**2 + delta**2)  # Detuning suppression factor
    prod_term = np.prod(1.0 + np.exp(-beta_m * gamma))
    exp_s = np.exp(-beta_s * omega)
    exp_m_sum = np.exp(-beta_m * gamma.sum())
    num = 1.0 + eta*(prod_term**(-1)) * (exp_s - exp_m_sum)
    den = (1.0 + exp_s) - eta* (prod_term**(-1)) * (exp_s - exp_m_sum) -1
...
E2 = 0.5*1.5
E3 = 0.75*1.5
beta_s_grid = np.linspace(0.5, 2, 300)
...
for idx, beta_m in enumerate(beta_m_values):
    for gamma, label, color in zip(gammas, gamma_labels, colors):
        y = np.array([beta_probe_post_detuned(omega, beta_s, beta_m, gamma, g=g) for beta_s in beta_s_grid])
...
textstr = (
    fr'$E_1={a}$, $E_2={b}$' + "\n" + 
    fr'$\omega={omega}, \beta_M ={beta_m_values[0]}$, $g={g}$' + "\n" +
...
save_path = "BV_temps_with_detuning.png"
